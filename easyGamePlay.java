	...
	public void actionPerformed(ActionEvent e) {
		if (play) {
			if (new Rectangle(x_pos, y_pos, 20, 20).intersects(new Rectangle(playerX, 550, 100, 8))) {  //블록 만들기
				...
				A: for (int i = 0; i < easymap.map.length; i++) {
					for (int j = 0; j < easymap.map[0].length; j++) {
						if (easymap.map[i][j] > 0) {
							int brickX = j * easymap.brickWidth + 80;
							int brickY = i * easymap.brickHeight + 50;
							int brickWidth = easymap.brickWidth;
							int brickHeight = easymap.brickHeight;
							Rectangle rect = new Rectangle(brickX, brickY, brickWidth, brickHeight);
							Rectangle ballRect = new Rectangle(x_pos, y_pos, 20, 20);
							Rectangle brickRect = rect;
						if (ballRect.intersects(brickRect)) {
							easymap.setBrickValue(0, i, j);
							totalBricks--;
							score += 5;
